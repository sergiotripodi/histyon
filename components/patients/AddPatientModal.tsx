...
export function AddPatientModal({ dict }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in">
      <div className="bg-white w-full max-w-2xl p-8 shadow-xl border border-gray-200 max-h-[90vh] overflow-y-auto relative">
        ...
        <div className="flex justify-between items-start mb-8">
          ...
          <button onClick={() => setIsOpen(false)} className="p-2 bg-white border border-gray-200 hover:border-gray-900 transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form action={handleSubmit} className="space-y-6" noValidate>
          ...
          <div className="space-y-4">
            <h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest flex items-center gap-2">
              <User className="w-3.5 h-3.5" /> {tf.sections.identity}
            </h4>
          ...
          <div className="space-y-4">
            <h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest flex items-center gap-2">
              <Phone className="w-3.5 h-3.5" /> {tf.sections.contacts}
            </h4>
            <div className="grid grid-cols-2 gap-4">
              <ValidatedInput name="email" label={tf.labels.emailSimple} type="email" placeholder={tf.placeholders.emailPlaceholder} pattern={REGEX_VALIDATORS.EMAIL} errorMessage={dict.validation.emailInvalid} required />
          ...
          <div>
            <h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest flex items-center gap-2 mb-4">
              <MapPin className="w-3.5 h-3.5" /> {tf.sections.domicile}
            </h4>

...
export default async function AdminUsersPage({ searchParams }: { searchParams: Promise<{ metric?: string }> }) {
  ...
  const [
    ...
    supabaseAdmin.from('tickets')
      .select('doctor_id, file_size, status, created_at')
      .order('created_at', { ascending: false }),
    supabaseAdmin.from('tickets')
  ...
  // Aggregate analyses count + storage bytes per user
  ...
  for (const t of tickets) {
    ...
    storageByUser[uid] = (storageByUser[uid] ?? 0) + size
  }
    ...
    <div className="py-10 px-8">
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        {[
          ...
          <div key={label} className="border border-gray-200 bg-white px-6 py-4">
      ...
      {/* Users table — moved up, this is the most useful view */}
      <div className="border border-gray-200 bg-white mb-8">
        <div className="px-6 py-4 border-b border-gray-100">
          <p className="text-[10px] font-medium uppercase tracking-[0.14em] text-gray-400">
            Ultimi 50 utenti registrati · analisi e spazio per utente
          </p>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100">
              {['Nome', 'Analisi', 'Storage', 'Email', 'Ospedale', 'Registrato il'].map(h => (
